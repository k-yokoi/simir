...
def train_10_fold_balanced(dv, tags):
    ...
    for train_idx, test_idx in skf.split(Xl, y):
        ...
        fold_path = os.path.join("pvdbow", str(fold_index))
        if os.path.exists("pvdbow") is not True:
            os.mkdir("pvdbow")
...
def predict_on_full_dataset(documents, tags):
    ...
    model = K.models.load_model('pvdbow/4/model.ckpt')
